...
func (tm *TombManager) StartBackgroundTask(taskFn func(ctx context.Context), taskName string) {
	...
	go func() {
		...
		logging.Info("Starting database background task: %s for %s", taskName, tm.name)
		taskFn(tm.ctx)
		logging.Info("Database background task completed: %s for %s", taskName, tm.name)
	}()
}
...
func (tm *TombManager) Shutdown() {
	logging.Info("Shutting down database background tasks for %s", tm.name)
	...
	logging.Info("All database background tasks stopped for %s", tm.name)
}
